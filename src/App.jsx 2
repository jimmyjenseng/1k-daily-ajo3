          ...
          <div className="relative">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-[#6b7280] text-sm mt-1.5">Hand #{collectorHand.no} &middot; Day {elapsed + 1} of {TOTAL_HANDS}</p>
              </div>
...
function AdminTab({
  ...
  const [form,         setForm]         = useState({ handNo: '', amount: DAILY_AMT, note: '', date: todayKey() });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setForm({ handNo: '', amount: DAILY_AMT, note: '', date: todayKey() });
    ...
    <div className="flex flex-col gap-4 animate-fade-in">
      ...
      <div className="card p-5 flex flex-col gap-4">
        ...
        <div className="flex gap-2">
          ...
          {editingId && <button className="btn-ghost px-4" onClick={() => { setEditingId(null); setForm({ handNo: '', amount: DAILY_AMT, note: '', date: todayKey() }); }}>Cancel</button>}
        </div>
      </div>
                  ...
                  return (
                    <div key={p.id} className="card2 p-3 flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button onClick={() => { setEditingId(p.id); setForm({ handNo: String(p.handNo), amount: p.amount, note: p.note || '', date: p.date }); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
